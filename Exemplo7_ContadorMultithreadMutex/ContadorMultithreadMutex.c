...
int main()
{
  ...
  DWORD id_thread;
  Mutex_Acesso_Contador = CreateMutex(NULL, FALSE, "Mutex_Acesso_Contador");
...
DWORD WINAPI Contar(LPVOID params)
{
  ...
  {
    WaitForSingleObject(Mutex_Acesso_Contador, INFINITE);
    contador++;
    ReleaseMutex(Mutex_Acesso_Contador);
  }
  return 0;
}
